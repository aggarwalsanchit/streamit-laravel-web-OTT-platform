...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const handleWatchButtonClick = async (button, isSeasonWatch = false) => {
    ...
    fetch(`${baseUrl}/api/continuewatch-list`)
      ...
      .then(async (data) => {
        ...
        const episodeIdAttr = button.getAttribute('data-episode-id')
        // continue_watch rows always store episodes under entertainment_type "tvshow" (see
        // saveContinueWatch()), never "episode", and share entertainment_id across every
        // episode of the same show, so episodes must additionally match on episode_id.
        const storedType = entertainmentType === 'episode' ? 'tvshow' : entertainmentType
        const matchingVideo = data.data.find((item) => {
          if (item.entertainment_id !== parseInt(entertainmentId) || item.entertainment_type !== storedType) {
            return false
          }
          if (storedType === 'tvshow') {
            return episodeIdAttr && item.episode_id === parseInt(episodeIdAttr)
          }
          return true
        })
  ...
  // Periodically persist watch progress during playback. Without this, progress was only
  // ever saved on 'ended' or on explicit forward/backward seeks, so a viewer who watched an
  // episode partway and simply navigated away (e.g. clicked another episode) left no
  // continue_watch row for that episode at all.
  let periodicSaveInterval = null;
  player.on('play', function () {
    if (periodicSaveInterval) clearInterval(periodicSaveInterval);
    periodicSaveInterval = setInterval(() => {
      if (contentVideoType == 'clip' || contentVideoType == 'trailer') return;
      const duration = player.duration();
      const currentTime = player.currentTime();
      if (!duration || currentTime < 5) return;
      saveWatchTimeImmediately(currentTime, duration);
    }, 15000);
  });
