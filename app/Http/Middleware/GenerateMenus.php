...
class GenerateMenus
{
    ...
    public function handle()
    {
        ...
        // Short Drama — admin only, and only when addon/routes are available.
               if (
                auth()->check()
                && auth()->user()->hasRole('admin')
                && \App\Services\ShortDrama\ShortDramaAddonStatus::isPresent()
                && \Illuminate\Support\Facades\Route::has('backend.short-drama-analytics.index')
            ) {
                $shortDrama = $this->parentMenu($menu, [
                    'icon'     => 'ph ph-film-strip',
                    'title'    => 'Short Dramas',
                    'nickname' => 'short_drama',
                    'order'    => 0,
                ]);

                $this->childMain($shortDrama, [
                    'icon'       => 'ph ph-chart-line-up',
                    'title'      => 'Analytics',
                    'route'      => 'backend.short-drama-analytics.index',
                    'active'     => ['app/short-drama/analytics'],
                    'shortTitle' => 'A',
                    'order'      => 1,
                ]);
                ...
                $this->childMain($shortDrama, [
                    ...
                    'title'      => 'Dramas',
                    'route'      => 'backend.short-dramas.index',
                    'active'     => ['app/short-drama/dramas', 'app/short-drama/dramas/*'],
                    'shortTitle' => 'D',
                    'order'      => 2,
                ]);
                ...
                $this->childMain($shortDrama, [
                    'icon'       => 'ph ph-cards-three',
                    'title'      => 'Episodes',
                    'route'      => 'backend.short-drama-episodes.index',
                    'active'     => ['app/short-drama/episodes', 'app/short-drama/episodes/*'],
                    'shortTitle' => 'E',
                    'order'      => 3,
                ]);

                if (\Illuminate\Support\Facades\Route::has('backend.transcode.index')) {
                    $this->childMain($shortDrama, [
                        'icon'       => 'ph ph-gear',
                        'title'      => 'Transcoding Status',
                        'route'      => 'backend.transcode.index',
                        'active'     => ['app/short-drama/transcode', 'app/short-drama/transcode/*'],
                        'shortTitle' => 'T',
                        'order'      => 3.5,
                    ]);
                }
                ...
                $this->childMain($shortDrama, [
                    ...
                    'route'      => 'backend.short-drama-sections.index',
                    'active'     => ['app/short-drama/sections', 'app/short-drama/sections/*'],
                    'shortTitle' => 'S',
                    'order'      => 4,
                ]);
                ...
                $this->childMain($shortDrama, [
                    'icon'       => 'ph ph-image',
                    'title'      => 'Banners',
                    'route'      => 'backend.short-drama-banners.index',
                    'active'     => ['app/short-drama/banners', 'app/short-drama/banners/*'],
                    'shortTitle' => 'B',
                    'order'      => 5,
                ]);
                ...
                $this->childMain($shortDrama, [
                    'icon'       => 'ph ph-gear-six',
                    'title'      => __('messages.short_drama_sidebar_featured_setting'),
                    'route'      => 'backend.short-drama-settings.index',
                    'active'     => ['app/short-drama/settings'],
                    'shortTitle' => 'FS',
                    'order'      => 5,
                ]);
    ...
    'order' => 0,
            ...
            if (auth()->user()->hasRole('admin')) {
                $this->mainRoute($menu, [
                    ...
                    'title'      => __('sidebar.plugin'),
                    'route'      => 'backend.short-drama.installer',
                    'active'     => ['app/short-drama/installer'],
                    'shortTitle' => __('sidebar.plugin_short'),
                    'order'      => 0,
                ]);
            }
