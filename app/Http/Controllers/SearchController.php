    ...
    public function check_in_trash(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'customads':
                ...
                break;
            case 'short-drama':
                $shortDramaModel = 'Modules\\ShortDrama\\Models\\ShortDrama';
                if (class_exists($shortDramaModel)) {
                    $InTrash = $shortDramaModel::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
                } else {
                    $InTrash = collect();
                }
                break;
            case 'short-drama-episode':
                $episodeModel = 'Modules\\ShortDrama\\Models\\ShortDramaEpisode';
                if (class_exists($episodeModel)) {
                    $InTrash = $episodeModel::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
                } else {
                    $InTrash = collect();
                }
                break;
            case 'short-drama-section':
                $sectionModel = 'Modules\\ShortDrama\\Models\\ShortDramaContentSection';
                if (class_exists($sectionModel)) {
                    $InTrash = $sectionModel::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
                } else {
                    $InTrash = collect();
                }
                break;
            case 'short-drama-banner':
                $shortDramaBannerModel = 'Modules\\ShortDrama\\Models\\ShortDramaBanner';
                if (class_exists($shortDramaBannerModel)) {
                    $InTrash = $shortDramaBannerModel::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
                } else {
                    $InTrash = collect();
                }
                break;
