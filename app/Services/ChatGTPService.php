...
class ChatGTPService
{
    ...
    private function generateMessage($name, $description = '', $type = null, ?int $maxChars = null)
    {
        if ($maxChars !== null) {
            $limit = "Maximum {$maxChars} characters (strict limit, plain text only, no HTML).";
            $userMessage = $description
                ? "Generate a short description for {$name}. Context: {$description}. {$limit} Language is English US."
                : "Generate a short description for {$name}. {$limit} Creativity is {$this->creativity} between 0 and 1. Language is English US.";
    ...
    public function GenerateDescription($name, $description, $type = null, ?int $maxChars = null)
    {
        $messages = $this->generateMessage($name, $description, $type, $maxChars);
