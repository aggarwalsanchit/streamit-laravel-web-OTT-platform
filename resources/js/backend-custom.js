...
(function () {
  ...
  if (typeof $.fn.select2 !== typeof undefined) {
    $('.select2').not('[data-select2-manual]').select2();
    $('.select2-tag').select2({
      tags: true
    });
  }
  ...
  const showBackendSessionFlash = () => {
    const flash = window.__backendFlash
    if (!flash || typeof flash !== 'object') {
      return
    }
    const errorText = flash.error
    const successText = flash.success ?? flash.message
    if (errorText != null && String(errorText).trim() !== '' && typeof window.errorSnackbar === 'function') {
      window.errorSnackbar(String(errorText))
    } else if (successText != null && String(successText).trim() !== '' && typeof window.successSnackbar === 'function') {
      window.successSnackbar(String(successText))
    }
    try {
      delete window.__backendFlash
    } catch (e) {
      window.__backendFlash = null
    }
  }
  showBackendSessionFlash()
